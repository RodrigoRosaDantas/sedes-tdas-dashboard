...
export async function loadData(){const r=await fetch(BASE+'data/site.json?v=1',{cache:'no-store'});if(!r.ok)throw new Error('Falha ao carregar dados ('+r.status+')');return r.json()}
...
export function setupShell(page,meta){
 ...
 document.addEventListener('click',e=>{
  ...
  if(theme){const next=document.documentElement.dataset.theme==='light'?'dark':'light';document.documentElement.dataset.theme=next;localStorage.setItem('edas-theme',next)}
  const dl=e.target.closest('[data-download]');
  if(dl){download(dl.dataset.download,window.__EDAS_DATA__)}
 });
 if('serviceWorker'in navigator)navigator.serviceWorker.register(BASE+'sw.js?v=1').catch(()=>{});
}
function toCSV(rows){if(!rows.length)return'';const keys=[...new Set(rows.flatMap(Object.keys))];const q=v=>`"${String(v??'').replaceAll('"','""')}"`;return [keys.map(q).join(','),...rows.map(r=>keys.map(k=>q(r[k])).join(','))].join('\n')}
