...
self.addEventListener('activate',event=>event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith('edas-')&&key!==VERSION).map(key=>caches.delete(key)));const cache=await caches.open(VERSION);await Promise.all(RESERVED_DATA.map(url=>cache.delete(url)));await self.clients.claim()})()));
