...
if(catalog.mode==='operational-empty'){
 ...
 required(catalog.keyPath===null&&(catalog.keyRef==null)&&catalog.authorizedSource===null&&catalog.peId===null,'Catálogo de preparação contém fonte, correção ou PE.');
 ...
 if(catalog.mode==='notion-daily'){
  if(answerService.mode==='private')required(catalog.keyPath==null&&/^daily\/tdas-pe\d+-[a-f0-9]+$/i.test(catalog.keyRef||''),'Referência privada da correção inválida.');
  else required(/^data\/integration\/question-keys\/pe\d+\.json$/i.test(catalog.keyPath||''),'Caminho legado da correção inválido.');
 }
}
...
required(scripts.player.includes('loadAnswerKey')&&scripts.answerClient.includes('SAFE_KEY_REF')&&scripts.answerClient.includes('SAFE_LEGACY_PATH'),'Player não restringe a referência da correção.');
const finishPosition=scripts.player.indexOf('async function finishSession'),keyFetchPosition=scripts.player.indexOf('loadAnswerKey(state.catalog',finishPosition),savePosition=scripts.player.indexOf("mode:'study'",finishPosition);
...
required(scripts.bank.includes('loadAnswerKey')&&scripts.answerClient.includes('privateItems'),'Banco não restringe as origens de correção.');
...
for(const asset of ['assets/v27.css','assets/integration/question-bank.js','assets/integration/question-bank-player.js','assets/integration/master-bank-ui.js','assets/integration/bank-draft-guard.js','assets/integration/review-catalog-bridge.js','assets/integration/resolver-bootstrap.js','assets/integration/continuity-engine.js','assets/integration/home-v27.js'])required(sw.includes(asset)&&preserveV27.includes(`'${asset}'`),`Asset v27/v28 sem blindagem PWA: ${asset}.`);
...
required(!precacheLists.includes('question-keys/')&&!postprocess.includes("'data/integration/question-keys/")&&!preserveV27.includes("'data/integration/question-keys/"),'Correção foi incluída no precache.');
required(sw.includes('purgePublicAnswerKeys')&&sw.includes('isPublicAnswerKey'),'PWA não remove gabaritos legados do cache.');
required(sw.includes('assets/integration/answer-key-client.js')&&sw.includes('data/integration/answer-key-service.json'),'Cliente privado e configuração estão fora do PWA.');
