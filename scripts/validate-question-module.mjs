...
required(scripts.bootstrap.includes("params.get('modo')==='banco'")&&scripts.bootstrap.includes('question-bank-player.js?v=1.0.0'),'Modo Banco não está isolado no roteador.');
required(scripts.bootstrap.includes('review-catalog-bridge.js?v=1.0.0'),'Revisão histórica não usa a ponte de catálogo.');
...
for(const label of ['Resolver questões','Banco de questões','Revisar','Caderno de erros','Desempenho','Fila de IA'])required(more.includes(label),`Menu Mais não preserva ${label}.`);
...
for(const asset of ['assets/v27.css','assets/integration/question-bank.js','assets/integration/question-bank-player.js','assets/integration/bank-draft-guard.js','assets/integration/review-catalog-bridge.js','assets/integration/resolver-bootstrap.js','assets/integration/continuity-engine.js','assets/integration/home-v27.js'])required(sw.includes(asset)&&preserveV27.includes(`'${asset}'`),`Asset v27 sem blindagem PWA: ${asset}.`);
...
required(!sw.includes('question-keys/')&&!postprocess.includes('question-keys/')&&!preserveV27.includes('question-keys/'),'Correção foi incluída no precache.');
...
console.log(`Módulo validado: conteúdo diário ${catalog.mode}, seis rotas, Banco v27, armazenamento v2, revisão adaptativa e correção carregada somente ao finalizar.`);
