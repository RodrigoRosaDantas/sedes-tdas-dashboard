...
required(scripts.store.includes('buildReinforcementReview')&&scripts.store.includes('reviewOutcome'),'Armazenamento não preserva o histórico adaptativo legado.');
...
required(scripts.player.includes('data-review-outcome')&&scripts.player.includes('Dominei')&&scripts.player.includes('Ainda tenho dúvida')&&scripts.player.includes('Errei novamente'),'Player não preserva compatibilidade histórica com tentativas antigas de revisão.');
...
required(decisionPosition>=0&&scripts.player.indexOf('saveCompletedAttempt',finishPosition)>decisionPosition,'Compatibilidade histórica de revisão pode ser salva antes da decisão pedagógica.');
...
required(preserveHistory.includes('question-archive/index.json')&&preserveHistory.includes('Object.values(archive.catalogs||{})'),'Arquivo histórico não é preservado dinamicamente no PWA.');
...
console.log(`Módulo validado: conteúdo diário ${catalog.mode}, seis rotas, Banco Mestre local, armazenamento v2, revisão externa por prioridades e correção carregada somente ao finalizar.`);