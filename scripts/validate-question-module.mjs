...
for(const asset of ['daily-execution.js','daily-law.js','today-execution.js','daily-question-page.js','daily-progress.js','local-backup.js','daily-content.css','module-dashboard.js','module-player.js','module-store.js','module-reviews.js','module-error-book.js','module-performance.js','module-ai-queue.js','player-core.js','player.css'])required(sw.includes(asset)&&postprocess.includes(asset),`Asset fora do PWA: ${asset}.`);
...
required(packageData.scripts?.check?.includes('validate-daily-content.mjs')&&packageData.scripts?.check?.includes('validate-question-module.mjs')&&packageData.scripts?.check?.includes('test-question-module.mjs'),'Validação integral fora do gate principal.');
console.log(`Módulo validado: conteúdo diário ${catalog.mode}, seis rotas, armazenamento v2 e correção carregada somente ao finalizar.`);
