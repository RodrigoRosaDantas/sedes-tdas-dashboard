...
required(scripts.bootstrap.includes('local-only-result-policy.js')&&!scripts.bootstrap.includes('attempt-diagnostics.js'),'Resolver deve usar resultado transitório sem diagnóstico persistente.');
...
for(const invariant of ['daily-execution-pe-links','theory-and-questions-separated','no-example-question-bank','master-bank-local-snapshot-only','no-master-bank-remote-runtime-read','local-storage-v2-only','no-notion-writeback'])required(navigation.invariants.includes(invariant),`Invariante ausente: ${invariant}.`);
required(scripts.store.includes("tdas.202.question-module.v2.state"),'Namespace local v2 ausente para leitura de legado.');
required(scripts.store.includes('persisted:false')&&scripts.store.includes('cloudSync:false')&&!scripts.store.includes('setItem(STORAGE_KEY'),'Conclusão de questões voltou a persistir histórico pessoal.');
required(scripts.reviewEngine.includes("MASTERED:'mastered'")&&scripts.reviewEngine.includes("UNSURE:'unsure'")&&scripts.reviewEngine.includes("WRONG_AGAIN:'wrong_again'"),'Resultados pedagógicos históricos obrigatórios ausentes.');
required(scripts.reviewEngine.includes('Reforço 24h')&&scripts.reviewEngine.includes('Reforço 3d'),'Motor histórico de revisão foi removido dos dados locais.');
required(scripts.reviewEngine.includes('sortReviewsByPriority')&&scripts.reviewEngine.includes('reviewPriorityScore'),'Motor adaptativo não preserva priorização como contrato de dados legado.');
...
const finishPosition=scripts.player.indexOf('async function finishSession'),keyFetchPosition=scripts.player.indexOf('state.catalog.keyPath',finishPosition);
...
required(scripts.bankPlayer.includes('saveCompletedAttempt')&&scripts.bankPlayer.includes('writeSessionDraft'),'Banco não preserva rascunho local e fechamento transitório.');
...
for(const asset of ['daily-execution.js','daily-law.js','today-execution.js','daily-question-page.js','daily-progress.js','local-backup.js','daily-content.css','module-dashboard.js','module-player.js','module-store.js','review-engine.js','module-reviews.js','module-error-book.js','module-performance.js','module-ai-queue.js','player-core.js','player.css'])required(sw.includes(asset)&&postprocess.includes(asset),`Asset legado fora do PWA: ${asset}.`);
...
required(preserveHistory.includes('REMOVE_ASSETS')&&preserveHistory.includes('firebase-history-store.js')&&preserveHistory.includes('local-only-result-policy.js'),'Guarda pós-sync não remove histórico pessoal do PWA.');
...
console.log(`Módulo validado: conteúdo diário ${catalog.mode}, seis rotas, Banco Mestre local, fechamento transitório local-only, revisão externa por prioridades e correção carregada somente ao finalizar.`);