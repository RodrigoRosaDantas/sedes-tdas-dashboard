...
const scripts={dashboard:await read('assets/integration/module-dashboard.js'),player:await read('assets/integration/module-player.js'),store:await read('assets/integration/module-store.js'),reviews:await read('assets/integration/module-reviews.js'),errors:await read('assets/integration/module-error-book.js'),performance:await read('assets/integration/module-performance.js'),queue:await read('assets/integration/module-ai-queue.js'),daily:await read('assets/integration/daily-execution.js'),todayDaily:await read('assets/integration/today-execution.js'),questionDaily:await read('assets/integration/daily-question-page.js')};
const catalog=await json('data/integration/question-catalog.json'),daily=await json('data/integration/daily-execution.json'),navigation=await json('data/integration/navigation.json'),manifest=await json('manifest.webmanifest'),packageData=await json('package.json');
const sw=await read('sw.js'),postprocess=await read('scripts/postprocess-v26.mjs'),home=await read('assets/home.js'),more=await read('assets/more.js'),morePage=await read('mais/index.html'),enhance=await read('assets/enhance-v20.js');
...
required(daily.mode==='daily-execution-contract'&&daily.materialPageIds.length===112&&daily.questionPageIds.length===112,'Contrato diário ausente ou incompleto.');
required(navigation.mode==='daily-execution-integrated'&&navigation.routes.length===6,'Navegação do módulo divergente.');
for(const invariant of ['daily-execution-pe-links','theory-and-questions-separated','no-example-question-bank','no-master-bank-runtime-read','authorized-catalog-only','local-storage-v2-only','no-notion-writeback'])required(navigation.invariants.includes(invariant),`Invariante ausente: ${invariant}.`);
...
const finishPosition=scripts.player.indexOf('async function finishSession'),keyFetchPosition=scripts.player.indexOf('state.catalog.keyPath');
...
required(!/api\.notion|method\s*:\s*['"](?:POST|PUT|PATCH|DELETE)/i.test(Object.values(scripts).join('\n')),'Módulo contém writeback ou API do Notion.');
required(scripts.daily.includes('daily-execution.json')&&scripts.dashboard.includes('materialUrl')&&scripts.questionDaily.includes('questionsUrl'),'Fluxo diário não liga material e questões.');
...
for(const asset of ['daily-execution.js','today-execution.js','daily-question-page.js','module-dashboard.js','module-player.js','module-store.js','module-reviews.js','module-error-book.js','module-performance.js','module-ai-queue.js','player-core.js','player.css'])required(sw.includes(asset)&&postprocess.includes(asset),`Asset fora do PWA: ${asset}.`);
for(const dataFile of ['data/integration/navigation.json','data/integration/question-catalog.json','data/integration/daily-execution.json'])required(sw.includes(dataFile)&&postprocess.includes(dataFile),`Dado fora do PWA: ${dataFile}.`);
...
required(packageData.scripts?.check?.includes('validate-daily-execution.mjs')&&packageData.scripts?.check?.includes('validate-question-module.mjs')&&packageData.scripts?.check?.includes('test-question-module.mjs'),'Validação integral fora do gate principal.');
console.log('Módulo validado: execução diária integrada, seis rotas funcionais, armazenamento v2 e nenhum conteúdo de exemplo ativo.');
