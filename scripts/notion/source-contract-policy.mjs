...
export function buildContractAssessment({controls=[],microDays=[],catalog=null,currentPe='',snapshotDate=''}){
 ...
 for(const [pe,control]of byControl){
  ...
  if(micro.expectation.mode==='strict'||micro.expectation.mode==='rest'){
   if(!matchesExpectation(count,micro.expectation))conflicts.push({code:'control_vs_micro',pe,severity:severityFor({pe,currentPe,date:control.date,snapshotDate}),message:`${pe}: Controle prevê ${count??'—'} questões; Micro exige ${micro.expectation.min===micro.expectation.max?micro.expectation.min:`${micro.expectation.min}–${micro.expectation.max}`}.`});
 ...
 if(currentMicro&&catalog&&['strict','rest'].includes(currentMicro.expectation.mode)&&!matchesExpectation(catalogCount,currentMicro.expectation))conflicts.push({code:'catalog_vs_micro',pe:currentPe,severity:'critical',message:`${currentPe}: catálogo publicou ${Number.isFinite(catalogCount)?catalogCount:'—'} questões; Micro exige ${currentMicro.expectation.min===currentMicro.expectation.max?currentMicro.expectation.min:`${currentMicro.expectation.min}–${currentMicro.expectation.max}`}.`});
