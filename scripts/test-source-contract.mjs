...
const pe100Micro=parseMicroMarkdown(`## PE100 — 25/08/2026 — Português final + RD29
<table><tr><td>Questões do tema principal — Português final</td><td>35</td></tr><tr><td>Microdose Português</td><td>8</td></tr><tr><td>Microdose Específicos peso 2</td><td>8</td></tr><tr><td>Total estimado</td><td>51</td></tr></table>`,15);
const canonicalPe100=buildContractAssessment({controls:[{pe:'PE100',date:'2026-08-25',planned_questions:'60'}],microDays:pe100Micro,catalog:{peId:'PE100',questionCount:60,authorizedSource:{url:'https://example.test/pe100-final',resolution:'validated-child-page'}},currentPe:'PE100',snapshotDate:'2026-08-25'});assert.equal(canonicalPe100.status,'ready_with_warnings');assert.equal(canonicalPe100.current.status,'ready');assert.deepEqual(canonicalPe100.current.conflicts.map(item=>item.code).sort(),['canonical_catalog_overrides_micro','canonical_control_overrides_micro']);assert.equal(canonicalPe100.current.conflicts.every(item=>item.severity==='warning'),true);
const mismatchedCanonical=buildContractAssessment({controls:[{pe:'PE100',date:'2026-08-25',planned_questions:'60'}],microDays:pe100Micro,catalog:{peId:'PE100',questionCount:59,authorizedSource:{url:'https://example.test/pe100-final',resolution:'validated-child-page'}},currentPe:'PE100',snapshotDate:'2026-08-25'});assert.equal(mismatchedCanonical.status,'blocked');
const unvalidatedPrimary=buildContractAssessment({controls:[{pe:'PE100',date:'2026-08-25',planned_questions:'60'}],microDays:pe100Micro,catalog:{peId:'PE100',questionCount:60,authorizedSource:{url:'https://example.test/pe100',resolution:'primary-page'}},currentPe:'PE100',snapshotDate:'2026-08-25'});assert.equal(unvalidatedPrimary.status,'blocked');
