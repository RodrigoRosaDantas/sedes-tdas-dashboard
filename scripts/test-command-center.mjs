...
assert.ok(reviews.includes('A revisão pedagógica acontece no ChatGPT.'));
assert.ok(reviews.includes('TDAS → ChatGPT → Notion'));
assert.ok(!reviews.includes('Iniciar revisão prioritária'),'A rota Revisar não deve recriar a agenda adaptativa como ação principal.');
...
console.log(`Fases 1–3 validadas: central, retomada, fechamento oficial, sinais adaptativos preservados como dados, handoff ChatGPT + Notion, última sincronização ${platform.syncAt}, publicação ${platform.publicationId} e ${missing.length} origens pendentes sem inferência ou writeback.`);
