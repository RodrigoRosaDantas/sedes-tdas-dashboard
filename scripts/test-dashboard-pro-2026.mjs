...
assert.match(versionSync, /VISUAL_CACHE_REV='cachefix3-pro9'/, 'Gerador deve usar a revisão visual PRO9.');
assert.match(platform.serviceWorkerVersion, /cachefix3-pro9$/, 'Manifesto publicado deve invalidar o cache visual anterior.');
assert.match(sw, /cachefix3-pro9/, 'Service worker deve usar a revisão visual PRO9.');
assert.ok(!sw.includes('question-keys/'), 'Gabaritos devem continuar fora do precache.');

console.log('Dashboard PRO 2026 unificado: uma Home, decisão operacional direta, dados oficiais, PWA PRO9 e responsividade preservados.');
