...
async function inspect(p){
 const client=await page(p);await nav(client,base);await waitFor(client,"document.documentElement.dataset.homeV28==='1'",`Home v28 ${p.name}`);await delay(450);
 return evalJs(client,`(()=>{
  ...
  const blocks=[...document.querySelectorAll('[data-v28-block]')].filter(visible).map(n=>n.dataset.v28Block);
  const headings=[...document.querySelectorAll('[data-v28-block] h2')].filter(visible).map(n=>({text:n.textContent.trim(),client:n.clientHeight,scroll:n.scrollHeight,overflow:getComputedStyle(n).overflow}));
  ...
  const system=document.querySelector('[data-v28-block="system"]');
  ...
  return {
   ...
   blocks,heroActions,primaryHref:primary?.href||'',centerHref:centerPrimary?.href||'',headings,mainOffenders,
...
try{
 ...
 for(const p of profiles){
  ...
  assert.deepEqual(d.blocks,['status','risks','performance','upcoming','library','system'],`${p.name}: ordem dos seis blocos v28 incorreta.`);
  ...
  assert.equal(d.systemVisible,true,`${p.name}: bloco Dados e sistema deve permanecer visível.`);
 ...
 console.log('Auditoria responsiva TDAS v28 aprovada: mobile, iPad retrato/paisagem e desktop sem overflow, truncamento ou alvo de toque crítico.');
