...
try{
 ...
 for(const [width,height] of [[390,844],[1024,768],[1366,1024]]){
  ...
  assert.match(d.systemText,/snapshot/i,'Barra de publicação deve identificar o snapshot vigente.');
  assert.match(d.systemText,/sincroniza|publica|atualiza/i,'Barra de publicação deve comunicar o estado da atualização.');
 ...
 console.log('Home pós-prova v2 validada no Chrome: próximo marco, timeline, memória do ciclo, arquivo, publicação e zero dashboard pré-prova.');
