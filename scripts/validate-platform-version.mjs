...
for(const field of['platformVersion','dataVersion','catalogVersion','serviceWorkerVersion','syncDate','peId']){
...
assert.equal(manifest.publicationId,[manifest.platformVersion,manifest.dataVersion,manifest.catalogVersion,manifest.syncDate,commitRef].join('|'),'publicationId inconsistente.');
...
console.log(`Versão consolidada validada: plataforma ${manifest.platformVersion}, dados ${manifest.dataVersion}, ${manifest.peId}, cache ${manifest.serviceWorkerVersion}.`);
