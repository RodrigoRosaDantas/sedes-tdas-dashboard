...
const [pkgText,watchdog,browser,globalBrowser,sw,player,docs,siteText,catalogText,keyText]=await Promise.all([
 read('package.json'),read('.github/workflows/edas-publication-watchdog.yml'),read('.github/workflows/edas-browser-smoke.yml'),read('.github/workflows/browser-smoke.yml'),read('edas-administracao/sw.js'),read('edas-administracao/assets/integration/module-player.js'),read('docs/OPERACAO_SITE_TDAS.md'),read('edas-administracao/data/site.json'),read('edas-administracao/data/integration/question-catalog.json'),read('edas-administracao/data/integration/answer-key.json')
]);
...
const globalMatches=(globalBrowser.match(/'edas-administracao\/\*\*'/g)||[]).length;assert.ok(globalMatches>=2,'Browser global deve reagir a alterações do EDAS em PR e push.');
...
assert.equal(site.meta.version,key.status==='ready'?site.meta.version:site.meta.version);
