...
const sw=read('sw.js');
const generator=read('scripts/postprocess-v26.mjs');
...
for(const [source,label] of [[sw,'service worker publicado'],[generator,'gerador diário do service worker']]){
 assert.match(source,/USER_CACHE_PREFIXES=\['tdas-redactions-user-'\]/,`O ${label} deve declarar a família de caches pessoais.`);
 assert.match(source,/shouldPreserveCache/,`O ${label} deve preservar caches pessoais.`);
 assert.match(source,/filter\(key=>!shouldPreserveCache\(key\)\)/,`O ${label} não pode apagar o cache de redações do usuário.`);
}
...
console.log(`Guardas P0.1 validadas: gerador idempotente, cache persistente, sincronização verificável, acessibilidade e versão ${version}.`);
