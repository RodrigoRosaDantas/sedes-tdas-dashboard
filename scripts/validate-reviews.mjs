...
const [scheduler, store, player, pageScript, reviewPage, attempts, contracts, packageText] = await Promise.all([
...
for (const classification of ['incorrect_confirmed','correct_with_doubt','correct_by_guess','marked']) {
  required(scheduler.includes(`'${classification}'`), `Classificação revisável ausente: ${classification}.`);
}
for (const excluded of ['correct_secure','annulment_pending','source_error']) {
  required(!scheduler.match(new RegExp(`REVIEW_ELIGIBLE_CLASSIFICATIONS[\\s\\S]{0,300}'${excluded}'`)), `Classificação indevida na agenda: ${excluded}.`);
}
...
required(player.includes('scheduleAttemptReviews') && player.includes('completeReview') && player.includes('getReviewById'), 'Player não integra criação e conclusão das revisões.');
required(player.includes('if (isReviewMode())') && player.includes('else {\n      state.reviewStatus = scheduleAttemptReviews'), 'Fluxos piloto e revisão não estão separados.');
required(player.includes('{includeD0: false}'), 'Player não mantém D0 desativado.');
required(player.includes("mode: isReviewMode() ? 'review' : 'pilot'"), 'Modo da tentativa não acompanha o contexto.');
required(player.includes('não gerou uma nova sequência de revisões'), 'Interface não declara o bloqueio de recursão.');
required(packageText.includes('check:reviews') && packageText.includes('test:reviews'), 'Comandos das revisões ausentes.');

console.log('Revisões validadas: D+1/D+7/D+20, D0 opt-in, agenda local, player de uma questão e ausência de recursão/writeback.');
