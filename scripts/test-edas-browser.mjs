...
const chrome=spawn(chromeBin,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--remote-debugging-address=127.0.0.1',`--remote-debugging-port=${port}`,`--user-data-dir=${profile}`,'about:blank'],{stdio:['ignore','ignore','pipe']});
...
try{
 ...
 const [site,catalog]=await Promise.all([waitJson(`${base}/data/site.json`,60),waitJson(`${base}/data/integration/question-catalog.json`,60)]);
