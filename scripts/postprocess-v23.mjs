...
await write('data/live-v23.json', overlay);

const swPath = path.join(ROOT, 'sw.js');
let sw = await fs.readFile(swPath, 'utf8');
sw = sw.replace(/const VERSION='[^']+';/, `const VERSION='tdas-v23-${String(snapshotDate || '').replaceAll('-', '')}-official';`);
const livePath = '/sedes-tdas-dashboard/data/live-v23.json';
if (!sw.includes(livePath)) {
  const marker = '"/sedes-tdas-dashboard/data/live.json"';
