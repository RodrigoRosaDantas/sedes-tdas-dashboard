...
const required = (condition, message) => { if (!condition) throw new Error(`PWA do módulo: ${message}`); };
const sw = await read('sw.js');
const manifest = JSON.parse(await read('manifest.webmanifest'));
const packageData = JSON.parse(await read('package.json'));
const list = name => {
  const match = sw.match(new RegExp(`const ${name}=\\[([\\s\\S]*?)\\];`));
  required(match, `lista ${name} ausente`);
...
const requiredAssets = ['assets/integration/module-dashboard.js','assets/integration/module-player.js','assets/integration/module-store.js','assets/integration/module-reviews.js','assets/integration/module-error-book.js','assets/integration/module-performance.js','assets/integration/module-ai-queue.js','assets/integration/player-core.js','assets/integration/player.css'];
const requiredData = ['data/integration/navigation.json','data/integration/question-catalog.json'];
required(/^tdas-v26-module-\d{8}-questions$/.test(version), `versão inválida: ${version}`);
...
console.log(`PWA validado: ${requiredRoutes.length} rotas e ${requiredAssets.length} módulos reais, sem catálogo de exemplo.`);
