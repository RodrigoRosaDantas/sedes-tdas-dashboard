...
async function inspect(width){const client=await page(width);await nav(client,base);await waitFor(client,"document.querySelector('.tdas-home-focus')&&document.querySelector('.tdas-command-trigger')&&document.querySelector('.tdas-performance-svg')",`Home carregada em ${width}px`);await delay(300);return evalJs(client,`(()=>{const box=node=>{if(!node)return null;const r=node.getBoundingClientRect();return{left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height}};const rect=s=>box(document.querySelector(s));const topbar=rect('.topbar'),crumb=rect('.topbar .crumb'),actions=rect('.topbar .actions'),chart=rect('.tdas-performance-svg'),chartCard=rect('.tdas-performance-card'),metrics=[...document.querySelectorAll('.tdas-home-metrics .metric')].map(box),heads=[...document.querySelectorAll('.tdas-dashboard-section .section-head')].map(box),badge=document.querySelector('.topbar .tdas-publication-badge'),hrefs=[...document.styleSheets].map(x=>String(x.href||''));return{width:innerWidth,doc:document.documentElement.scrollWidth,body:document.body.scrollWidth,topbar,crumb,actions,chart,chartCard,metrics,heads,badge:badge?getComputedStyle(badge).display:null,hotfix:Boolean(hrefs.some(x=>x.includes('home-mobile-hotfix.css'))),styleCounts:{v20:hrefs.filter(x=>x.includes('/assets/v20.css')).length,mobile:hrefs.filter(x=>x.includes('/assets/tdas-mobile-ux.css')).length,hotfix:hrefs.filter(x=>x.includes('/assets/home-mobile-hotfix.css')).length}}})()`)}
...
try{
 ...
 for(const width of[360,390,430]){
  ...
  console.log(`HOME_LAYOUT_${width}=${JSON.stringify({topbar:d.topbar,crumb:d.crumb,actions:d.actions,doc:d.doc,body:d.body,chart:d.chart,chartCard:d.chartCard,styles:d.styleCounts})}`);
  assert.equal(d.hotfix,true,`Hotfix mobile deve estar carregado em ${width}px.`);
  assert.equal(d.styleCounts.v20,1,`Home deve carregar v20.css uma única vez em ${width}px.`);
  assert.equal(d.styleCounts.mobile,1,`Home deve carregar tdas-mobile-ux.css uma única vez em ${width}px.`);
  assert.equal(d.styleCounts.hotfix,1,`Home deve carregar o hotfix mobile uma única vez em ${width}px.`);
 ...
 console.log('Home mobile validada em 360, 390 e 430px: sem overflow, sem CSS duplicado, cabeçalho sem colisão e gráfico responsivo.');
