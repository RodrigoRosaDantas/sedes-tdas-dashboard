...
async function inspect(width){const client=await page(width);await nav(client,base);await waitFor(client,"document.querySelector('.pro26-dashboard')&&document.querySelector('.pro26-pulse')&&document.querySelector('.pro26-line-chart')",`Home unificada carregada em ${width}px`);await delay(300);return evalJs(client,`(()=>{const box=node=>{if(!node)return null;const r=node.getBoundingClientRect();return{left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height}};const rect=s=>box(document.querySelector(s));const topbar=rect('.topbar'),crumb=rect('.topbar .crumb'),actions=rect('.topbar .actions'),chart=rect('.pro26-pulse-chart .pro26-line-chart'),chartCard=rect('.pro26-pulse-chart'),metrics=[...document.querySelectorAll('.pro26-metric')].map(box),heads=[...document.querySelectorAll('.pro26-plan>header,.pro26-analytics>header,.pro26-list-card>header')].map(box),badge=document.querySelector('.topbar .tdas-publication-badge'),hrefs=[...document.styleSheets].map(x=>String(x.href||''));return{width:innerWidth,doc:document.documentElement.scrollWidth,body:document.body.scrollWidth,topbar,crumb,actions,chart,chartCard,metrics,heads,badge:badge?getComputedStyle(badge).display:null,styles:{v20:hrefs.filter(x=>x.includes('/assets/v20.css')).length,mobile:hrefs.filter(x=>x.includes('/assets/tdas-mobile-ux.css')).length,unified:hrefs.filter(x=>x.includes('/assets/dashboard-pro-2026.css')).length,hotfix:hrefs.filter(x=>x.includes('/assets/home-mobile-hotfix.css')).length,v27:hrefs.filter(x=>x.includes('/assets/v27.css')).length}}})()`)}
...
try{
 ...
 for(const width of[360,390,430]){
  ...
  console.log(`HOME_LAYOUT_${width}=${JSON.stringify({topbar:d.topbar,crumb:d.crumb,actions:d.actions,doc:d.doc,body:d.body,chart:d.chart,chartCard:d.chartCard,styles:d.styles})}`);
  ...
  assert.equal(d.styles.unified,1,`Home deve carregar dashboard-pro-2026.css uma única vez em ${width}px.`);
  ...
  if(d.badge!==null)assert.equal(d.badge,'none',`Status textual deve sair do cabeçalho estreito em ${width}px e permanecer no drawer.`);
  assert.ok(d.crumb&&d.actions&&d.crumb.right<=d.actions.left+1,`Identidade e ações do topo não podem se sobrepor em ${width}px: ${JSON.stringify({crumb:d.crumb,actions:d.actions,topbar:d.topbar})}`);
  assert.ok(d.actions.right<=width+1,`Busca/menu devem caber no cabeçalho em ${width}px.`);
  assert.ok(d.chart&&d.chartCard&&d.chart.width<=d.chartCard.width+1,`Gráfico deve ser responsivo em ${width}px.`);
  assert.ok(d.metrics.length>=4&&d.metrics.every(box=>box.left>=-1&&box.right<=width+1),`Cards de métricas devem permanecer dentro da viewport em ${width}px.`);
  assert.ok(d.heads.length>=2&&d.heads.every(box=>box.left>=-1&&box.right<=width+1),`Cabeçalhos de seção devem permanecer dentro da viewport em ${width}px.`);
 }
 console.log('Home unificada mobile validada em 360, 390 e 430px: sem overflow, sem overlays legados, cabeçalho sem colisão e gráfico responsivo.');
