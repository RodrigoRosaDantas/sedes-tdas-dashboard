import{loadJSON,setupShell,fmtNumber,fmtPct,fmtDate,metric,alertCard,setLoadingError,routes}from'./common.js?v=21';
...
function examCountdown(examDate){
 ...
 return{
  days,
  detail:parts.join(' e '),
  headline:days===0?'A prova é hoje':`Faltam ${days} ${days===1?'dia':'dias'} para a prova`
 };
}
...
try{
 ...
 <section class="hero">
  ...
  <div class="hero-actions"><a class="btn primary" href="${routes.hoje}">Abrir foco de hoje</a><a class="btn" href="${routes.evolucao}">Ver evolução</a></div>
 ...
 <section class="grid metrics">
  ...
  ${metric('Questões concluídas',fmtNumber(m.questions),'meta operacional')}
  ${metric('Acertos registrados',fmtNumber(m.correct),`${fmtNumber(2223)} questões com resultado`)}
  ${metric('Aproveitamento',fmtPct(m.accuracy),'dias com resultado preenchido')}
 </section>
 <section class="section">
  ...
  <div class="grid two">
   <a class="card focus-main" href="${routes.hoje}"><span class="kicker">Em andamento</span><h2>${d.today.title}</h2><div class="tags"><span class="tag">${d.today.meta} questões</span><span class="tag">${d.today.block}</span><span class="tag">${d.today.rd}</span></div><span class="btn primary">Abrir plano do dia →</span></a>
 ...
 <footer class="footer"><span>TDAS · Plataforma v20</span><span>Sincronização: <span data-sync></span></span></footer>`;
