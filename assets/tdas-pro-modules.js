...
const pageKey=()=>{const path=location.pathname;if(path.startsWith(BASE+'resolver/'))return'resolver';if(path.startsWith(BASE+'revisar/'))return null;if(path.startsWith(BASE+'caderno-erros/'))return'caderno';if(path.startsWith(BASE+'desempenho/'))return'desempenho';if(path.startsWith(BASE+'estudar/'))return'estudar';if(path.startsWith(BASE+'materias/'))return'materias';return null};
const config={
 resolver:{title:'Resolver questões',caption:'Sessão cega · correção ao finalizar',trail:[['1','Preparar','Ler o bloco'],['2','Resolver','Responder sem gabarito'],['3','Corrigir','Classificar e diagnosticar']]},
 caderno:{title:'Caderno de erros',caption:'Corrigir · transformar erro em decisão',trail:[['1','Detectar','Erro confirmado'],['2','Classificar','Causa e reincidência'],['3','Direcionar','Definir prioridade externa']]},
 ...
 estudar:{title:'Conteúdo',caption:'Executar · material antes da bateria',trail:[['1','Material','Teoria e lei seca'],['2','Questões','Aplicação cega'],['3','Fechar','Resultado e diagnóstico']]},
 materias:{title:'Biblioteca',caption:'Conteúdo · diagnóstico por matéria',trail:[['1','Escolher','Matéria prioritária'],['2','Diagnosticar','Padrões e conteúdo'],['3','Praticar','Questões e erros']]}
};
...
function pendingSignals(state,now=Date.now()){return(state.reviews||[]).filter(item=>item.status==='pending'&&Number(item.dueAt)<=now)}
...
async function buildScorecard(key){
 const state=readModuleState(),signals=pendingSignals(state),causes=Object.values(readCauses()),stats=attemptStats(state);
 if(key==='resolver'){
  ...
  return score('Questões',fmt(catalog?.questions?.length||0),catalog?.peId||'catálogo atual',true)+score('Sessão',draft?'Em andamento':'Pronta',draft?'continuidade preservada':'sem resposta iniciada')+score('Tentativas',fmt(stats.attempts.filter(item=>item.mode==='study').length),'neste dispositivo')+score('Sinais pendentes',fmt(signals.length),'para diagnóstico externo');
 }
 if(key==='caderno'){
  const errors=state.errors||[],classified=causes.filter(item=>item?.cause).length;
  return score('Erros locais',fmt(errors.length),'confirmados no player',true)+score('Reincidências',fmt(recurrentTopics(errors)),'tópicos repetidos')+score('Causas',fmt(classified),errors.length?`${Math.round(classified/errors.length*100)}% classificados`:'sem erros')+score('Marcações',fmt((state.marked||[]).length),'para diagnóstico');
 }
 if(key==='desempenho')return score('Aproveitamento',pct(stats.accuracy),`${fmt(stats.correct)}/${fmt(stats.questions)} corretas`,true)+score('Tentativas',fmt(stats.attempts.length),'sessões registradas')+score('Questões',fmt(stats.questions),'respondidas localmente')+score('Sinais pendentes',fmt(signals.length),'para priorização externa');
 if(key==='estudar'){
...
function crossNav(key){const items=[['resolver','Resolver','Executar questões'],['revisar','Prioridades','Escolher foco'],['caderno','Erros','Entender reincidência'],['desempenho','Progresso','Medir e agir']].filter(([id])=>id!==key);const href={resolver:'resolver/',revisar:'revisar/',caderno:'caderno-erros/',desempenho:'desempenho/'};return`<nav class="tdas-pro-crossnav" data-pro-crossnav aria-label="Atalhos do ciclo">${items.map(([id,title,detail])=>`<a href="${BASE}${href[id]}"><small>${esc(detail)}</small><b>${esc(title)} →</b></a>`).join('')}<a href="${BASE}"><small>Central de comando</small><b>Faça agora →</b></a></nav>`}
function nextCommand(key,state){
 const signals=pendingSignals(state),errors=state.errors||[];
 if(key==='caderno'&&recurrentTopics(errors)>0)return{title:'Ataque reincidências antes de seguir',detail:`${recurrentTopics(errors)} tópico${recurrentTopics(errors)===1?'':'s'} reapareceu mais de uma vez neste dispositivo.`,href:`${BASE}revisar/`,label:'Ver prioridades'};
 if(key==='desempenho'&&signals.length)return{title:'Seu dado já aponta uma prioridade',detail:`Há ${signals.length} sinal${signals.length===1?'':'s'} local${signals.length===1?'':'is'} que merece${signals.length===1?'':'m'} diagnóstico antes de aumentar o volume.`,href:`${BASE}revisar/`,label:'Ver prioridades'};
 ...
 if(key==='materias')return{title:'Use a biblioteca para decidir, não para passear',detail:'Abra a matéria com maior concentração de erros e volte ao ciclo por questões ou prioridades.',href:`${BASE}riscos/`,label:'Ver maior risco'};
