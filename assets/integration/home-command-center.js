...
function cycleTaskCard(task,active){const state=task.done?'Concluída no registro oficial':task.id==='next'?'Próximo ciclo':active?'Ação prioritária':'Pendente no ciclo oficial';return`<a class="command-cycle-task ${task.done?'done':''} ${active?'active':''}" href="${task.href}"><span>${task.done?'✓':task.id==='redaction'?'RD':task.id==='official-reviews'?'P':'→'}</span><div><b>${escapeHTML(task.label)}</b><small>${escapeHTML(state)}</small><p>${escapeHTML(task.detail)}</p></div></a>`}
try{
