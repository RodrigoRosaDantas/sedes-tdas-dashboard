...
try{
 const[{hero},home,contract,audit]=await Promise.all([waitForHome(),fetch(BASE+'data/home.json',{cache:'no-store'}).then(r=>r.json()),loadDailyExecution(),fetch(BASE+'data/audit.json',{cache:'no-store'}).then(r=>r.json())]);
