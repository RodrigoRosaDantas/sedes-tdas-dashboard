...
import {findCompletedPe87Complement,isPe87ComplementDraft,isPe87ComplementRequired,PE87_COMPLEMENT} from './pe87-complement.js?v=1.0.0';
...
try{
 ...
 let action=selectPrimaryAction({pe,progress:effective,draft,attempt,nextPe,dueReview,overduePe,officialCompleted,officialTasks,base:BASE});
 const complementRequired=isPe87ComplementRequired(home.today),completedComplement=findCompletedPe87Complement(local.attempts,platform.catalogVersion),complementDraft=isPe87ComplementDraft(draft,platform.catalogVersion),complementPending=complementRequired&&!completedComplement;
 if(complementPending){
  action={stage:'complement',label:complementDraft?'Continuar complemento PE87 · Q31–Q48':'Completar PE87 · Q31–Q48',detail:`O histórico oficial Q1–Q${PE87_COMPLEMENT.historicalAnswered} permanece 23/30. Execute somente as ${PE87_COMPLEMENT.total} questões adicionais antes de seguir para a revisão.`,href:`${BASE}resolver/?pe=PE87&mode=complement${complementDraft?'&resume=1':''}`,button:complementDraft?'Continuar complemento':'Iniciar complemento'};
 }
