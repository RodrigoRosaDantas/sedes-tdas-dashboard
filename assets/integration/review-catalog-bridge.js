...
import {loadMasterQuestionBank} from './question-bank.js?v=1.1.0';
...
const installCatalogOverride=(catalog,kind)=>{
 ...
 document.documentElement.dataset.reviewCatalog=kind;
 return catalog;
};
export async function installReviewCatalogBridge(reviewId){
 ...
 const master=await loadMasterQuestionBank();const question=(master.questions||[]).find(item=>String(item.id)===String(review.questionId));
 if(!question)return null;
 const catalog={schemaVersion:'2.1.0',mode:'master-review',catalogId:`master-review:${question.id}`,title:`Revisão · ${question.materialName||'Banco Mestre'}`,description:'Questão recuperada do snapshot publicado do Banco Mestre TDAS.',peId:'BANCO',questionCount:1,suggestedMinutes:2,keyPath:question.sourceKeyPath,authorizedSource:{type:'master-question-bank',repository:master.snapshot?.source?.repository||null,commit:master.snapshot?.source?.commit||null},questions:[question]};
