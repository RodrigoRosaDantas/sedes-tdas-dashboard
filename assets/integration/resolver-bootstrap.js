...
const addBankSwitch=()=>{
 ...
 const actions=main.querySelector('.hero-actions');if(!actions||actions.querySelector('[data-bank-switch]'))return;
 const link=document.createElement('a');link.className='btn';link.dataset.bankSwitch='1';link.href=BASE+'resolver/?modo=banco';link.textContent='Banco de questões';actions.append(link);
};
if(reviewId){
 ...
 const observer=new MutationObserver(addBankSwitch);observer.observe(document.querySelector('main')||document.body,{childList:true,subtree:true});addBankSwitch();
}