...
try{
 ...
 if(contextNode){
  contextNode.innerHTML=`<div class="section-head"><div><span class="kicker">Contexto preservado</span><h2>${escapeHTML(focus||'Evidências para análise')}</h2><p>${focus?'O foco vindo do Mentor foi preservado para a análise no ChatGPT.':'O TDAS mantém os sinais locais como evidência, sem transformar D+1, D+7 ou D+20 em agenda pedagógica automática.'}</p></div></div><div class="grid metrics"><article class="card metric"><small>Respostas divergentes</small><strong>${errors.length}</strong><span>evidências brutas</span></article><article class="card metric"><small>Marcações</small><strong>${marked.length}</strong><span>itens sinalizados</span></article><article class="card metric"><small>Sinais locais vencidos</small><strong>${due.length}</strong><span>preservados, não preemptivos</span></article></div>${focus?`<article class="card panel"><h3>Levar ao ChatGPT: ${escapeHTML(focus)}</h3><p>${subject&&mentorFocus?`${escapeHTML(subject)} · `:''}Compare respostas divergentes, recorrência e regra envolvida. Classifique a causa real e decida se a intervenção é teoria, lei seca ou nova bateria.</p><div class="hero-actions"><a class="btn primary" href="${attempt?exportHref(attempt):`${BASE}caderno-erros/`}">${attempt?'Preparar última tentativa':'Abrir evidências'}</a><a class="btn" href="${BASE}mentor/">Voltar ao Mentor</a></div></article>`:''}${legacyReviewId?`<p class="muted">Origem legada preservada: ${escapeHTML(legacyReviewId)}.</p>`:''}`;
 }
