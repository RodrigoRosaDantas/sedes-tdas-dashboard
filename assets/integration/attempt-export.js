...
import {loadAnswerKey} from './answer-key-client.js?v=1.0.0';
...
export function buildAttemptExport({attempt,questionsById={},user={},source='local'}={}){
 ...
 const questions=attempt.questionResults.map(result=>{
  ...
  const q={id:String(result.id),peId:attempt.peId||null,codigo:text(snapshot.codigo),numeroOriginal:result.numeroOriginal??snapshot.numeroOriginal??snapshot.numero_original??snapshot.numero??null,materia:text(snapshot.materia??snapshot.disciplina),assunto:text(result.assunto??snapshot.assunto),subassunto:text(result.subassunto??snapshot.subassunto),textoBase:text(snapshot.textoBase??snapshot.texto_base),enunciado:text(snapshot.enunciado),alternativas,selected,selectedText:text(snapshot.selectedText??alternativeText(alternativas,selected)),correctAnswer,correctText:text(snapshot.correctText??alternativeText(alternativas,correctAnswer)),correct:result.correct===true,confidence:text(result.confidence||'secure'),marked:result.marked===true,classification:text(result.classification),issue:text(result.issue||'none'),sourcePe:text(snapshot.sourcePe),sourceTitle:text(snapshot.sourceTitle),sourceKind:text(snapshot.sourceKind),sourceKeyRef:text(snapshot.sourceKeyRef),sourceKeyPath:text(snapshot.sourceKeyPath),sourcePublicPath:text(snapshot.sourcePublicPath),materialId:text(snapshot.materialId),materialName:text(snapshot.materialName),tipoMaterial:text(snapshot.tipoMaterial??snapshot.tipo_material),banca:text(snapshot.banca??snapshot.fonte),ano:snapshot.ano==null?null:Number(snapshot.ano),orgao:text(snapshot.orgao),cargo:text(snapshot.cargo),codigoCargo:text(snapshot.codigoCargo??snapshot.codigo_cargo),dificuldade:text(snapshot.dificuldade),formatoQuestao:text(snapshot.formatoQuestao??snapshot.formato_questao),comentario:text(snapshot.comentario),fundamento:text(snapshot.fundamento??snapshot.justificativa),pegadinha:text(snapshot.pegadinha),comentariosAlternativas:copyObject(snapshot.comentariosAlternativas??snapshot.comentarios_alternativas??null),fonteOficial:text(snapshot.fonteOficial??snapshot.fonte_oficial),observacoes:text(snapshot.observacoes),...telemetry};
...
export async function enrichAttemptExportWithCorrections(exported,{fetchFn=globalThis.fetch,base=BASE}={}){
 const data=validateAttemptExport(exported);if(typeof fetchFn!=='function'||!data.questions.some(q=>q.sourceKeyRef||q.sourceKeyPath))return data;
 let payload;try{payload=await loadAnswerKey({catalogId:data.attempt.catalogId||data.attempt.id,keyRef:null,questions:data.questions.map(q=>({id:q.id,sourceKeyRef:q.sourceKeyRef,sourceKeyPath:q.sourceKeyPath}))},{fetchFn,base})}catch{return data}
 const map=new Map((payload.answers||[]).filter(x=>x?.id).map(x=>[String(x.id),x]));if(!map.size)return data;
 const questions=data.questions.map(q=>{const detail=map.get(q.id);if(!detail)return q;const next={...q,comentario:text(detail.comentario??q.comentario),fundamento:text(detail.fundamento??q.fundamento),pegadinha:text(detail.pegadinha??q.pegadinha),comentariosAlternativas:copyObject(detail.comentariosAlternativas??detail.comentarios_alternativas??q.comentariosAlternativas),fonteOficial:text(detail.fonteOficial??detail.fonte_oficial??q.fonteOficial),observacoes:text(detail.observacoes??q.observacoes)};return{...next,diagnostico:diagnoseQuestion(next)}});
