...
const icon=kind=>({session:'▶',review:'↻',errors:'!',plan:'→'})[kind]||'→';
try{
 ...
 const fallback={href:defaultPrimary?.getAttribute('href')||BASE+'hoje/',label:defaultPrimary?.textContent?.trim()||'Abrir atividade de hoje',detail:hero.querySelector('.tdas-home-focus-copy')?.textContent?.trim()||'Próxima ação do ciclo oficial'};
