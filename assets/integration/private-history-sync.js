...
async function restoreNewestDraft(rows,catalog){
 const newest=(rows||[]).find(row=>row.status==='active'&&row.payload);if(!newest||!catalog||newest.catalog_id!==catalog.catalogId)return false;
 const local=readSessionDraft(),remoteAt=Date.parse(newest.client_updated_at)||0,localAt=Number(local?.savedAt||0);if(local&&localAt>=remoteAt)return false;
 try{writeSessionDraft(newest.payload);return true}catch{return false}
}
async function pullRemote(client,userId,catalog){
 const attempts=await client.from(TABLES.attempts).select('attempt_id,pe_id,finished_at,payload').eq('user_id',userId).order('finished_at',{ascending:false}).limit(500);if(attempts.error)throw attempts.error;
 for(const row of attempts.data||[])await dbPut(STORES.remote,{attemptId:row.attempt_id,peId:row.pe_id,finishedAt:Date.parse(row.finished_at),exported:row.payload,summary:row.payload?.attempt||null,syncedAt:Date.now()});
 ...
 const states=await client.from(TABLES.state).select('record_type,record_id,client_updated_at,device_id,payload').eq('user_id',userId).order('client_updated_at',{ascending:false}).limit(2000);if(states.error)throw states.error;
 for(const row of states.data||[])await dbPut(STORES.meta,{key:`remote-state:${row.record_type}:${row.record_id}`,value:row,updatedAt:Date.parse(row.client_updated_at)||Date.now()});
 const restoredDraft=await restoreNewestDraft(drafts.data,catalog);return{attempts:(attempts.data||[]).length,drafts:(drafts.data||[]).length,states:(states.data||[]).length,restoredDraft};
}
export async function syncPrivateHistory(){
 ...
 const client=await privateHistoryClient(),deviceId=getOrCreateDeviceId(),queue=await dbList(STORES.queue),due=queue.filter(x=>x.status!=='synced'&&Number(x.nextRetryAt||0)<=Date.now());let uploaded=0,failed=0;
 ...
 const pulled=await pullRemote(client,session.user.id,catalog);writeSyncMeta({lastSyncAt:new Date().toISOString(),deviceId,userId:session.user.id,uploaded,failed,remoteAttempts:pulled.attempts,remoteDrafts:pulled.drafts,remoteStates:pulled.states,restoredDraft:pulled.restoredDraft});return{status:failed?'partial':'synced',uploaded,failed,remote:pulled.attempts,drafts:pulled.drafts,states:pulled.states,restoredDraft:pulled.restoredDraft};
}
