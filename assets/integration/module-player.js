...
import {buildPe87ComplementCatalog} from './pe87-complement.js?v=1.0.0';

const main = document.querySelector('main');
const state = {catalog: null, session: null, responseMeta: {}, review: null, timer: null, draft: null, pendingReview: null, mode: 'study'};
const safeKeyPath = path => /^data\/integration\/question-keys\/[a-z0-9._-]+\.json$/i.test(String(path || ''));
const questionNumber = (question,index=0) => question?.numeroOriginal ?? question?.numero_original ?? index + 1;
...
function renderIntro(){
 ...
 const resume=state.draft&&!state.review,progress=resume?sessionProgress(state.draft.session):null,kicker=state.review?`Revisão ${escapeHTML(state.review.stage)}`:state.mode==='complement'?'Complemento PE87 · Q31–Q48':'Sessão local';
 main.innerHTML=`<section class="hero"><span class="kicker">${kicker}</span><h1>${escapeHTML(state.review?`Revisar questão ${state.review.numeroOriginal??''}`:catalog.title)}</h1><p>${escapeHTML(catalog.description||'Responda todas as questões antes da correção.')}</p><div class="tags"><span class="tag">${catalog.questions.length} ${catalog.questions.length===1?'questão':'questões'}</span>${state.mode==='complement'?'<span class="tag">Histórico Q1–Q30 preservado</span>':''}<span class="tag">Correção somente ao finalizar</span><span class="tag">Salvo neste dispositivo</span></div><div class="hero-actions">${resume?`<button class="btn primary" data-module-resume>Continuar questão ${questionNumber(catalog.questions[state.draft.session.currentIndex],state.draft.session.currentIndex)} · ${progress.answered}/${progress.total} respondidas</button><button class="btn" data-module-start>Recomeçar</button>`:'<button class="btn primary" data-module-start>Iniciar</button>'}<a class="btn" href="${state.review?`${BASE}revisar/`:BASE}">Voltar</a></div>${resume?`<p><small>${progress.answered} respostas preservadas. O gabarito continua indisponível até a finalização.</small></p>`:''}</section>`
}
function renderQuestion(){
 const catalog=questionCatalog(),progress=sessionProgress(state.session),question=catalog.questions[state.session.currentIndex],selected=state.session.answers[question.id]||'',meta=metaFor(question.id),number=questionNumber(question,state.session.currentIndex);
 main.innerHTML=`<section class="hero pilot-shell"><div class="pilot-toolbar"><div class="pilot-progress"><div><strong>Questão ${escapeHTML(number)} · ${state.session.currentIndex+1} de ${progress.total}</strong> · ${progress.answered} respondidas</div><div class="pilot-progress-track"><div class="pilot-progress-fill" style="width:${progress.percent}%"></div></div></div><strong class="pilot-timer" data-module-timer>${formatElapsed(Date.now()-state.session.startedAt)}</strong></div></section><section class="section pilot-shell"><article class="card panel pilot-question"><span class="kicker">${escapeHTML(question.assunto||'Questão')}</span><h1>${escapeHTML(question.enunciado)}</h1>${question.texto_base?`<blockquote class="pilot-text">${escapeHTML(question.texto_base)}</blockquote>`:''}<fieldset class="pilot-options"><legend class="skip">Escolha uma alternativa</legend>${ANSWER_OPTIONS.filter(option=>question.alternativas?.[option]).map(option=>`<label class="pilot-option"><input type="radio" name="module-answer" value="${option}" ${selected===option?'checked':''}><span><strong>${option})</strong> ${escapeHTML(question.alternativas[option])}</span></label>`).join('')}</fieldset><fieldset class="pilot-meta"><legend><strong>Como você chegou à resposta?</strong></legend><div class="pilot-meta-grid"><label><input type="radio" name="module-confidence" value="secure" ${meta.confidence==='secure'?'checked':''}> Segurança</label><label><input type="radio" name="module-confidence" value="doubt" ${meta.confidence==='doubt'?'checked':''}> Dúvida</label><label><input type="radio" name="module-confidence" value="guess" ${meta.confidence==='guess'?'checked':''}> Chute</label></div><label><input type="checkbox" data-module-marked ${meta.marked?'checked':''}> Marcar para revisão</label><label>Ressalva editorial<select data-module-issue><option value="none" ${meta.issue==='none'?'selected':''}>Nenhuma</option><option value="annulment_pending" ${meta.issue==='annulment_pending'?'selected':''}>Possível anulação</option><option value="source_error" ${meta.issue==='source_error'?'selected':''}>Possível erro da fonte</option></select></label></fieldset></article><article class="card panel"><h2>Mapa da sessão</h2><div class="pilot-map">${catalog.questions.map((item,index)=>`<button class="btn ${state.session.answers[item.id]?'answered':''} ${index===state.session.currentIndex?'current':''}" data-module-index="${index}">${escapeHTML(questionNumber(item,index))}</button>`).join('')}</div></article><div class="pilot-actions"><button class="btn" data-module-prev ${state.session.currentIndex===0?'disabled':''}>← Anterior</button><a class="btn" href="${state.review?`${BASE}revisar/`:BASE}">Sair</a>${state.session.currentIndex<progress.total-1?'<button class="btn primary" data-module-next>Próxima →</button>':`<button class="btn primary" data-module-finish ${canFinish(state.session)?'':'disabled'}>Finalizar (${progress.remaining} pendentes)</button>`}</div></section>`;updateTimer()
}
function renderStudyResult(evaluation,saved){
 const complement=state.mode==='complement';
 main.innerHTML=`<section class="hero pilot-result"><span class="kicker">${complement?'Resultado do complemento':'Resultado local'}</span><h1>${evaluation.correct}/${evaluation.total} acertos · ${evaluation.percent.toFixed(0)}%</h1><p>${complement?'Q31–Q48 foram registradas separadamente; o histórico Q1–Q30 permanece intacto.':'Tentativa salva somente neste dispositivo.'} Foram registrados ${saved.attempt.questionResults.filter(item=>item.classification==='incorrect_confirmed').length} erros nesta sessão.</p><div class="hero-actions"><a class="btn primary" href="${complement?BASE:`${BASE}resolver/`}">${complement?'Voltar à central':'Nova sessão'}</a><a class="btn" href="${BASE}caderno-erros/">Abrir caderno</a><a class="btn" href="${BASE}desempenho/">Ver desempenho</a></div></section>`
}
...
async function finishSession(){
 ...
 const selectedIds=new Set(catalog.questions.map(item=>item.id));
 const normalizedKey=(state.review||state.mode==='complement')?{...key,material_id:catalog.catalogId,answers:key.answers.filter(item=>selectedIds.has(item.id))}:key,evaluation=evaluateSession(state.session,normalizedKey,Date.now());
 stopTimer();
 if(state.review){renderReviewDecision(evaluation);return}
 const saved=saveCompletedAttempt({catalog,evaluation,responseMeta:state.responseMeta,mode:state.mode});clearSessionDraft();state.draft=null;renderStudyResult(evaluation,saved)
}
...
main.addEventListener('click',event=>{
 ...
 else if(event.target.closest('[data-module-finish]')){finishSession().catch(error=>alert(error.message));return}else return;
 persistDraft();renderQuestion()
});
try{
 const[baseCatalog,shell]=await Promise.all([fetch(BASE+'data/integration/question-catalog.json',{cache:'no-store'}).then(response=>response.json()),loadJSON('data/more.json')]);setupShell('mais',shell.meta);
 const params=new URLSearchParams(location.search),reviewId=params.get('review'),complementRequested=params.get('mode')==='complement';
 if(reviewId&&complementRequested)throw new Error('Revisão e complemento não podem ser executados na mesma sessão.');
 state.catalog=complementRequested?buildPe87ComplementCatalog(baseCatalog):baseCatalog;
 if(complementRequested&&!state.catalog)throw new Error('O complemento PE87 Q31–Q48 não está disponível no catálogo autorizado atual.');
 state.mode=complementRequested?'complement':'study';
 if(reviewId){state.review=readModuleState().reviews.find(item=>item.id===reviewId&&item.status==='pending')||null;if(!state.review)throw new Error('Revisão local não encontrada ou já concluída.');if(state.review.dueAt>Date.now())throw new Error('Esta revisão ainda não está disponível.');}
 else state.draft=matchingSessionDraft(state.catalog);
 if(!Array.isArray(state.catalog.questions)||state.catalog.questions.length===0)renderEmpty();else if(state.draft&&params.get('resume')==='1')resumeDraft();else renderIntro()
