...
try{
 const[catalog,shell]=await Promise.all([fetch(BASE+'data/integration/question-catalog.json',{cache:'no-store'}).then(response=>response.json()),loadJSON('data/more.json')]);setupShell('mais',shell.meta);state.catalog=catalog;
 const params=new URLSearchParams(location.search),reviewId=params.get('review');
 ...
 else state.draft=matchingSessionDraft(catalog);
 if(!Array.isArray(catalog.questions)||catalog.questions.length===0)renderEmpty();else if(state.draft&&params.get('resume')==='1')resumeDraft();else renderIntro()
