...
try {
  ...
  main.innerHTML = `
    <section class="hero">
      <span class="kicker">Desempenho local · piloto</span>
      <h1>Desempenho</h1>
      <p>Métricas reconstruídas das tentativas e revisões deste dispositivo, sem alterar o painel oficial da plataforma.</p>
      <div class="tags"><span class="tag">${snapshot.attempts} tentativas</span><span class="tag">${snapshot.reviews.due} revisões disponíveis</span><span class="tag">Escopo piloto local</span></div>
    ...
    <section class="grid metrics">
      ...
      ${metric('Tentativas', fmtNumber(snapshot.attempts), `${snapshot.pilotAttempts} piloto · ${snapshot.reviewAttempts} revisão`)}
    ...
    <section class="section"><div class="section-head"><div><h2>Últimas tentativas</h2><p>Até vinte registros, em ordem cronológica.</p></div></div><div class="grid two">${snapshot.trend.map(item => `<article class="card panel"><small>${escapeHTML(item.mode === 'review' ? 'Revisão' : 'Piloto')} · ${fmtDate(item.finishedAt)}</small><h3>${item.correct}/${item.total} · ${fmtPct(item.percent)}</h3><p>Tempo: ${fmtTime(item.elapsedMs)}</p></article>`).join('')}</div></section>` : `
    <section class="section"><article class="card panel"><h2>Nenhuma tentativa local</h2><p>Conclua o piloto PE76 para formar o painel. O desempenho oficial continua disponível na página Evolução.</p></article></section>`}
    ...
    <section class="section"><article class="card panel"><h2>Separação preservada</h2><p>Este painel não substitui a Evolução oficial, não atualiza PE e não grava no Notion. Todas as métricas acima são derivadas em leitura.</p></article></section>
    <footer class="footer"><span>Desempenho local · Fase 9</span><span>Snapshot <span data-snapshot></span></span></footer>`;
} catch (error) {
