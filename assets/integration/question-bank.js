...
export function flattenBankCatalogs(catalogs=[]){
 ...
 for(const catalog of catalogs){
  ...
  for(const question of catalog.questions){
   ...
   rows.push({...question,sourceCatalogId:text(catalog.catalogId),sourcePe:text(catalog.peId),sourceTitle:text(catalog.title),sourceKeyPath:text(catalog.keyPath),materia:questionMateria(question)});
  }
 }
 return rows;
}
...
export function bankFacets(questions=[]){return{pes:unique(questions.map(q=>q.sourcePe)),materias:unique(questions.map(questionMateria)),assuntos:unique(questions.map(q=>q.assunto))}}
export function filterBankQuestions(questions=[],filters={}){
 const pe=text(filters.pe),materia=text(filters.materia),assunto=text(filters.assunto),query=normalizeBankText(filters.query);
 return questions.filter(question=>{
  ...
  if(query){const haystack=normalizeBankText([question.id,question.sourcePe,questionMateria(question),question.assunto,question.subassunto,question.enunciado].join(' '));if(!haystack.includes(query))return false;}
  return true;
 });
}
...
export function buildBankCatalog(questions=[],options={}){
 ...
 return{schemaVersion:'2.1.0',mode:'question-bank',catalogId:`tdas-bank-${fingerprint}`,title:options.title||`Banco de questões — ${questions.length} itens`,description:'Bateria montada a partir dos catálogos autorizados. A correção continua oculta até a finalização.',peId:'BANCO',questionCount:questions.length,suggestedMinutes:Math.max(1,Math.round(questions.length*1.5)),questions:questions.map(question=>({...question}))};
}
...
export async function loadQuestionBank(){const catalogs=await loadAllCatalogs();return{catalogs,questions:flattenBankCatalogs(catalogs)}}
