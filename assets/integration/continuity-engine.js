...
export function buildContinuity({draft=null,moduleState={},fallback=null,now=Date.now()}={}){
 ...
 if(due.length&&!String(fallback?.href||'').includes('/revisar/')){
  queue.push({kind:'priorities',priority:40,label:`Ver prioridades · ${due.length} ${due.length===1?'sinal':'sinais'}`,detail:'Sinais locais para direcionar a revisão externa; não inicia sessão de revisão no TDAS.',href:`${BASE}revisar/`});
 }
 ...
 return{primary,queue:queue.slice(0,3),dueSignals:due.length,dueReviews:due.length,hasActiveSession:queue.some(item=>item.kind==='session')};
}
