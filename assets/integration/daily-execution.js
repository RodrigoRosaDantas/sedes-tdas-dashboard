...
export async function loadDailyExecution(){
 if(!contractPromise)contractPromise=fetch(BASE+'data/integration/daily-execution.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`Falha ao carregar a execução diária (${r.status}).`);return r.json()}).then(raw=>{
  ...
  const items=raw.materialPageIds.map((materialId,index)=>{const number=index+1;return Object.freeze({pe:`PE${String(number).padStart(2,'0')}`,number,week:Math.ceil(number/7),materialUrl:NOTION+materialId,lawUrl:LAW_LIBRARY_URL,questionsUrl:NOTION+raw.questionPageIds[index]})});
  return Object.freeze({...raw,lawLibraryUrl:LAW_LIBRARY_URL,items:Object.freeze(items)});
 });
...
if(typeof document!=='undefined')import('./daily-law.js?v=1.0.2').catch(error=>console.error('Lei Seca indisponível',error));
