...
try {
  ...
  const matchingDraft = draft || null;
  ...
  if (matchingDraft) {
    operationalHref = `${BASE}resolver/?pe=${encodeURIComponent(matchingDraft.peId || currentPe)}&resume=1`;
    operationalTitle = `Continuar questão ${number(matchingDraft.session?.currentIndex) + 1} de ${number(matchingDraft.session?.questionIds?.length)}`;
    operationalLabel = 'Continuar de onde parei';
    operationalStage = 'questions';
