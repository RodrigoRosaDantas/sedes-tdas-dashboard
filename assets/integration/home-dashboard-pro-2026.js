...
try {
  ...
  main.innerHTML = `<div class="pro26-dashboard">
    ...
    <section class="pro26-decision-grid"><article class="pro26-decision"><div class="pro26-orbit"><i></i><i></i><i></i></div><div class="pro26-mode"><span>MODO RETA FINAL</span><span>${escapeHTML(current.pe || 'TDAS')}</span><span>${escapeHTML(current.status || 'Em andamento')}</span></div><span class="pro26-eyebrow">ORIENTAÇÃO DE HOJE</span><h2><small>Prioridade:</small> ${escapeHTML(recommendation?.topic || `${topPattern} em ${topSubject.subject || 'seu Caderno de Erros'}`)}</h2><p>${escapeHTML(recommendation?.evidence || topSubject.recommendation || home.alerts?.[0]?.detail || 'Comece pelo erro mais recorrente, pratique e registre o resultado no Notion.')}</p><div class="pro26-why"><span>POR QUE ISSO VEM PRIMEIRO</span><p>${fmtNumber(topSubject.errors || metrics.errors)} erros em ${escapeHTML(topSubject.subject || 'áreas monitoradas')} · ${fmtNumber(topSubject.recurrent || 0)} reincidências · ${fmtNumber(critical)} tópicos críticos no edital.</p></div><div class="pro26-decision-actions"><a class="pro26-btn lime" href="${BASE}caderno-erros/">Abrir Caderno de Erros</a><a class="pro26-btn dark" href="${BASE}resolver/">Praticar questões</a><a class="pro26-btn dark" href="${escapeHTML(notionUrl)}" target="_blank" rel="noopener noreferrer">Registrar no Notion ↗</a></div></article><aside class="pro26-pulse"><header><div><span>PULSO RECENTE</span><strong>${escapeHTML(last.pe || 'Últimas execuções')}</strong></div><b>${last.accuracy ? fmtPct(last.accuracy) : '—'}</b></header>${lineChart(recent.slice(-8))}<div class="pro26-delta ${delta < 0 ? 'negative' : ''}"><span>${delta < 0 ? '↓' : '↑'}</span><p><strong>${delta >= 0 ? '+' : ''}${fmtPct(delta)}</strong> em relação à execução anterior.</p></div><button type="button" data-pro26-open-questions>Painel completo abaixo ↓</button></aside></section>
