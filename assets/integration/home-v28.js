...
function riskSection({home,today,edital,dueReviews}){
 ...
 <article class="card tdas-v28-risk-card" data-tone="error"><div class="tdas-v28-card-head"><span>Erros recorrentes</span><strong>${fmtNumber(home.metrics?.errors||0)}</strong></div><h3>${escapeHTML(primaryAlert?.title||'Erros catalogados para retorno')}</h3><p>${escapeHTML(primaryAlert?.detail||'Use o Caderno para transformar erro real em revisão dirigida.')}</p><div class="tdas-v28-mini-list">${recentErrors.slice(0,2).map(item=>`<span><b>${escapeHTML(item.subject||'Questão')}</b>${escapeHTML(item.title||'Erro recente')}</span>`).join('')||'<span>Nenhum erro recente no PE atual.</span>'}</div><a href="${BASE}caderno-erros/">Revisar erros →</a></article>
