...
function compactHero({hero,center,home,today}){
 ...
 if(quick){
  ...
  if(completed(current.status)){
   parts.push(`${correct}/${attempted||correct} acertos`,`${errors} resposta${errors===1?'':'s'} divergente${errors===1?'':'s'}`);
...
function riskSection({home,today,edital,dueReviews}){
 ...
 const section=document.createElement('section');section.className='tdas-dashboard-section tdas-v28-block tdas-v28-risks';section.dataset.v28Block='risks';section.innerHTML=`<div class="section-head"><div><span class="kicker">Prioridade real</span><h2>Onde posso perder pontos</h2><p>Sinais acionáveis: respostas divergentes, evidências locais e fragilidades do edital.</p></div></div><div class="tdas-v28-risk-grid">
 <article class="card tdas-v28-risk-card" data-tone="error"><div class="tdas-v28-card-head"><span>Registros do Caderno</span><strong>${fmtNumber(home.metrics?.errors||0)}</strong></div><h3>${escapeHTML(primaryAlert?.title||'Ocorrências catalogadas para retorno')}</h3><p>${escapeHTML(primaryAlert?.detail||'Use o Caderno para reunir as evidências e levá-las à análise no ChatGPT.')}</p><div class="tdas-v28-mini-list">${recentErrors.slice(0,2).map(item=>`<span><b>${escapeHTML(item.subject||'Questão')}</b>${escapeHTML(item.title||'Ocorrência recente')}</span>`).join('')||'<span>Nenhuma ocorrência recente no PE atual.</span>'}</div><a href="${BASE}caderno-erros/">Ver evidências →</a></article>
 <article class="card tdas-v28-risk-card" data-tone="review"><div class="tdas-v28-card-head"><span>Evidências locais</span><strong>${dueReviews}</strong></div><h3>${dueReviews?`${dueReviews} evidência${dueReviews===1?'':'s'} para analisar`:'Sem evidências vencidas'}</h3><p>${escapeHTML(review?.detail||'Sinais adaptativos são preservados como dados. A decisão pedagógica acontece no ChatGPT.')}</p><div class="tdas-v28-mini-list"><span><b>Sinal do ciclo</b>${escapeHTML(home.today?.action||'Conferir o fechamento e as evidências do PE atual.')}</span></div><a href="${BASE}revisar/">Analisar evidências →</a></article>
 <article class="card tdas-v28-risk-card" data-tone="edital"><div class="tdas-v28-card-head"><span>Pontos frágeis do edital</span><strong>${number(risk.critical)}</strong></div><h3>${number(risk.critical)} críticos · ${number(risk.attention)} em atenção</h3><p>${number(risk.no_evidence)} tópico${number(risk.no_evidence)===1?'':'s'} ainda sem evidência suficiente no raio-X, mesmo com cobertura concluída.</p><div class="tdas-v28-mini-list">${priority.map(item=>`<span><b>${escapeHTML(item.discipline||'Edital')}</b>${escapeHTML(item.topic||'Analisar agora')}</span>`).join('')||'<span>Nenhum tópico crítico listado.</span>'}</div><a href="${BASE}riscos/">Ver pontos frágeis →</a></article>
 </div>`;return section;
}
...
function librarySection({home,edital}){
 ...
 <a class="card tdas-v28-library-card" href="${BASE}caderno-erros/"><span>!</span><div><small>${fmtNumber(home.metrics?.errors||0)} registros</small><strong>Caderno de Erros</strong><p>Reúna respostas divergentes e recorrências para análise no ChatGPT.</p></div><b>›</b></a>
