...
function offlinePageUrl(detail){return new URL(`${BASE}redacoes/detalhe/?rd=${encodeURIComponent(detail.rd)}`,location.origin).href;}
function offlineDataUrl(detail){return new URL(`${BASE}data/redactions/${detail.rd.toLowerCase()}.json`,location.origin).href;}
function offlineResources(detail){
 const styleUrls=[...document.querySelectorAll('link[rel="stylesheet"]')].map(link=>link.href);
 return [...new Set([
  offlinePageUrl(detail),
  offlineDataUrl(detail),
  new URL(`${BASE}data/redactions.json`,location.origin).href,
  import.meta.url,
  new URL(`${BASE}assets/common.js`,location.origin).href,
  ...styleUrls
 ])];
}
function updateOfflineIndex(detail,saved,resources=[]){const index=offlineIndex();if(saved)index[detail.rd]={savedAt:new Date().toISOString(),theme:detail.meta?.theme||'',resources};else delete index[detail.rd];localStorage.setItem(OFFLINE_KEY,JSON.stringify(index));}
async function isSavedOffline(detail){
 if(!('caches'in window))return false;
 const index=offlineIndex();if(!index[detail.rd])return false;
 const cache=await caches.open(OFFLINE_CACHE);
 const required=[offlinePageUrl(detail),offlineDataUrl(detail)];
 const matches=await Promise.all(required.map(resource=>cache.match(resource)));
 const valid=matches.every(Boolean);
 if(!valid)updateOfflineIndex(detail,false);
 return valid;
}
async function saveOffline(detail){
 ...
 const cache=await caches.open(OFFLINE_CACHE);const resources=offlineResources(detail);
 const responses=await Promise.all(resources.map(async resource=>{const response=await fetch(resource,{cache:'reload'});if(!response.ok)throw new Error(`Falha ao armazenar ${resource} (${response.status}).`);return[resource,response];}));
 await Promise.all(responses.map(([resource,response])=>cache.put(resource,response.clone())));updateOfflineIndex(detail,true,resources);
}
async function removeOffline(detail){
 if(!('caches'in window))return;
 const cache=await caches.open(OFFLINE_CACHE);const entry=offlineIndex()[detail.rd];
 const shared=new Set([new URL(`${BASE}data/redactions.json`,location.origin).href,import.meta.url,new URL(`${BASE}assets/common.js`,location.origin).href,...[...document.querySelectorAll('link[rel="stylesheet"]')].map(link=>link.href)]);
 const resources=(entry?.resources||[offlinePageUrl(detail),offlineDataUrl(detail)]).filter(resource=>!shared.has(resource));
 await Promise.all(resources.map(resource=>cache.delete(resource)));updateOfflineIndex(detail,false);
}
...
try{
 ...
 if(detail.access?.locked){document.querySelector('main').innerHTML=`${pager(nav)}<section class="hero rd-detail-hero"><div><span class="kicker">Aplicação cega protegida</span><h1>${escapeHTML(detail.rd)} — ${escapeHTML(detail.meta.theme)}</h1><p>A proposta completa ainda não foi liberada para preservar o treino previsto no calendário.</p></div></section><section class="section"><article class="card panel rd-lock-card"><div class="rd-lock-icon">🔒</div><div><h2>Liberação em ${fmtDate(detail.access.unlockDate)}</h2><p>${escapeHTML(detail.access.reason)}</p><p>Você pode acompanhar tema, data e eixo; comando e textos motivadores permanecem ocultos até a data planejada.</p><a class="btn" href="${BASE}redacoes/?tab=produce">Voltar às propostas</a></div></article></section>${pager(nav)}<footer class="footer"><span>${escapeHTML(detail.rd)} · aplicação cega</span><span>Última atualização <span data-last-sync></span></span></footer>`;}
  ...
  const perf=detail.performance;const saved=await isSavedOffline(detail);
  document.querySelector('main').innerHTML=`
   ...
   ${detail.corrected?`
    <section class="section" id="performance"><div class="section-head"><div><h2>Desempenho por critério</h2><p>Rubrica de treino CAC, OT e DLP${detail.meta.correctionDate?` · correção em ${fmtDate(detail.meta.correctionDate)}`:''}.</p></div></div><div class="grid rd-criteria">${criterion('CAC',perf.criteria?.cac,'Conteúdo e atendimento ao comando')}${criterion('OT',perf.criteria?.ot,'Organização textual')}${criterion('DLP',perf.criteria?.dlp,'Domínio da língua portuguesa')}</div></section>
   ...
   <section class="section"><article class="card panel rd-privacy"><h3>Privacidade</h3><p>O registro editorial de origem não é exposto por link direto nesta interface. O conteúdo pessoal continua na publicação pública enquanto não houver autenticação privada.</p></article></section>
   ${pager(nav)}
