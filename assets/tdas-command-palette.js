...
let items=[],matches=[],selected=0,lastFocus=null,ready=false;
...
function localActionItems(){
 ...
 const out=[];
 if(draft){const progress=Object.keys(draft.session?.answers||{}).length,total=draft.session?.questionIds?.length||0;out.push({group:'Continuar',kind:'action',icon:'▶',label:`Continuar ${draft.peId||'sessão'}`,meta:`Questão ${Math.min(total,draft.session.currentIndex+1)} de ${total} · ${progress} respondidas`,href:`${BASE}resolver/?resume=1${draft.peId?`&pe=${encodeURIComponent(draft.peId)}`:''}`,keywords:'continuar retomar sessao interrompida rascunho'})}
...
async function remoteItems(){
 ...
 for(let number=1;number<=112;number++){const pe=`PE${String(number).padStart(2,'0')}`;out.push({group:'PEs',kind:'pe',icon:'◎',label:pe,meta:'Abrir execução diária',href:`${BASE}estudar/?pe=${pe}`,keywords:`pe ${number} plano execucao material questoes`})}
...
function renderShell(){if(document.querySelector('[data-command-overlay]'))return;const overlay=document.createElement('div');overlay.className='tdas-command-overlay';overlay.dataset.commandOverlay='';overlay.hidden=true;overlay.innerHTML=`<section class="tdas-command-dialog" role="dialog" aria-modal="true" aria-label="Comandos e busca TDAS"><div class="tdas-command-head"><span>⌕</span><input data-command-input autocomplete="off" spellcheck="false" placeholder="Digite uma ação, matéria ou PE…" aria-label="Buscar comando"><button class="tdas-command-close" type="button" data-command-close aria-label="Fechar">×</button></div><div class="tdas-command-meta"><span><kbd>↑↓</kbd> navegar <kbd>Enter</kbd> abrir</span><span data-command-count>Ações e atalhos mais úteis</span></div><div class="tdas-command-results" data-command-results></div><footer class="tdas-command-foot"><span>TDAS PRO · navegação sem perder contexto</span><span>Esc para fechar</span></footer></section>`;document.body.appendChild(overlay)}
...
async function init(){renderShell();renderTrigger();bind();const remotes=await remoteItems();remotes.forEach(item=>item._remote=true);items=[...remotes];ready=true;render('')}
