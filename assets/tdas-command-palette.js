...
const staticItems=[
 {group:'Ações',kind:'action',icon:'⌂',label:'Hoje',meta:'Voltar ao foco atual do TDAS',href:BASE+'hoje/',keywords:'inicio home hoje executar foco'},
 {group:'Ações',kind:'action',icon:'▶',label:'Questões',meta:'Abrir o player de questões',href:BASE+'resolver/',keywords:'questoes simulado resolver executar player'},
 {group:'Ações',kind:'risk',icon:'!',label:'Erros',meta:'Evidências locais para validar',href:BASE+'caderno-erros/',keywords:'erro erros evidencia evidencias validar caderno'},
 {group:'Ações',kind:'action',icon:'◆',label:'Mentor',meta:'Análise estratégica do estudo',href:BASE+'mentor/',keywords:'mentor analise estrategia prioridade'},
 {group:'Navegação',kind:'route',icon:'✓',label:'Check do Edital',meta:'Cobertura e raio-X do Cargo 202',href:BASE+'edital/',keywords:'edital check raio x risco topicos cobertura'},
 {group:'Navegação',kind:'route',icon:'!',label:'Riscos',meta:'Pareto e sinais de atenção',href:BASE+'riscos/',keywords:'risco riscos pareto prioridade'},
 ...
 {group:'Navegação',kind:'route',icon:'▥',label:'Desempenho',meta:'Métricas e histórico de execução',href:BASE+'desempenho/',keywords:'desempenho progresso estatistica historico'},
...
function localActionItems(){
 ...
 if(due[0])out.push({group:'Continuar',kind:'action',icon:'↻',label:`Analisar evidências · ${due.length} ${due.length===1?'item':'itens'}`,meta:'Sinais locais prontos para classificação no ChatGPT',href:`${BASE}revisar/?origem=palette`,keywords:'analisar evidencia evidencias chatgpt validar pendente'});
 if(errors.length)out.push({group:'Continuar',kind:'risk',icon:'!',label:`Validar ${errors.length} ${errors.length===1?'resposta':'respostas'}`,meta:'Abrir evidências brutas antes da classificação',href:BASE+'caderno-erros/',keywords:'validar respostas evidencias caderno'});
 return out;
}
async function remoteItems(){
 ...
 const out=[];
 for(const subject of subjects?.subjects||[])out.push({group:'Matérias',kind:'subject',icon:'▤',label:subject.subject,meta:`${subject.errors||0} registros de erro · ${subject.recurrent||0} recorrências`,href:`${BASE}materias/${subject.slug}/`,keywords:`materia ${subject.subject} ${(subject.top_patterns||[]).map(x=>x.pattern).join(' ')}`});
...
function render(query=''){
 ...
 results.innerHTML=matches.length?groupedMarkup(matches):'<div class="tdas-command-empty"><b>Nada encontrado</b><span>Tente PE88, Português, evidências, redação ou edital.</span></div>';
...
function renderShell(){if(document.querySelector('[data-command-overlay]'))return;const overlay=document.createElement('div');overlay.className='tdas-command-overlay';overlay.dataset.commandOverlay='';overlay.hidden=true;overlay.innerHTML=`<section class="tdas-command-dialog" role="dialog" aria-modal="true" aria-label="Comandos e busca TDAS"><div class="tdas-command-head"><span>⌕</span><input data-command-input autocomplete="off" spellcheck="false" placeholder="Digite uma ação, matéria ou PE…" aria-label="Buscar comando"><button class="tdas-command-close" type="button" data-command-close aria-label="Fechar">×</button></div><div class="tdas-command-meta"><span><kbd>↑↓</kbd> navegar <kbd>Enter</kbd> abrir</span><span data-command-count>Ações e atalhos mais úteis</span></div><div class="tdas-command-results" data-command-results></div><footer class="tdas-command-foot"><span>TDAS · atalhos sem mudar o fluxo de estudo</span><span>Esc para fechar</span></footer></section>`;document.body.appendChild(overlay)}
