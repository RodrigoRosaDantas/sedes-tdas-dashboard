...
try{
 ...
 <section class="section"><div class="section-head"><div><h2>Raio-X por disciplina</h2><p>Distribuição dos riscos dentro das ${fmtNumber(summary.total||topics.length)} unidades internas de controle.</p></div></div><div class="grid portal-grid edital-disciplines">${disciplines.map(disciplineCard).join('')}</div></section>
 <section class="section" id="topicos"><div class="section-head"><div><h2>Checklist completo do edital</h2><p>Pesquise e filtre por disciplina, bloco e nível de risco. A ordem inicial prioriza risco e prioridade do ciclo.</p></div><span class="stamp" id="result-count" aria-live="polite">${fmtNumber(topics.length)} itens</span></div>
